...
TYPE R_assay IS RECORD (
		...
		ASSAY_TITLE	ASSAY.ASSAY_TITLE%type,
...
TYPE R_project_context_item IS RECORD (
		...
		GROUP_PROJECT_CONTEXT_ID	PROJECT_CONTEXT_ITEM.GROUP_PROJECT_CONTEXT_ID%type,
		PROJECT_ID	PROJECT_CONTEXT_ITEM.PROJECT_ID%type,
		PROJECT_STEP_ID	PROJECT_CONTEXT_ITEM.PROJECT_STEP_ID%type,
		DISCRIMINATOR	PROJECT_CONTEXT_ITEM.DISCRIMINATOR%type,
...
TYPE R_run_context_item IS RECORD (
		RUN_CONTEXT_ITEM_ID	RUN_CONTEXT_ITEM.RUN_CONTEXT_ITEM_ID%type,
		GROUP_RUN_CONTEXT_ID	RUN_CONTEXT_ITEM.GROUP_RUN_CONTEXT_ID%type,
		EXPERIMENT_ID	RUN_CONTEXT_ITEM.EXPERIMENT_ID%type,
		RESULT_ID	RUN_CONTEXT_ITEM.RESULT_ID%type,
		DISCRIMINATOR	RUN_CONTEXT_ITEM.DISCRIMINATOR%type,
		ATTRIBUTE_ID	RUN_CONTEXT_ITEM.ATTRIBUTE_ID%type,
		VALUE_ID	RUN_CONTEXT_ITEM.VALUE_ID%type,
		EXT_VALUE_ID	RUN_CONTEXT_ITEM.EXT_VALUE_ID%type,
		QUALIFIER	RUN_CONTEXT_ITEM.QUALIFIER%type,
		VALUE_NUM	RUN_CONTEXT_ITEM.VALUE_NUM%type,
		VALUE_MIN	RUN_CONTEXT_ITEM.VALUE_MIN%type,
		VALUE_MAX	RUN_CONTEXT_ITEM.VALUE_MAX%type,
		VALUE_DISPLAY	RUN_CONTEXT_ITEM.VALUE_DISPLAY%type,
		VERSION	RUN_CONTEXT_ITEM.VERSION%type,
		DATE_CREATED	RUN_CONTEXT_ITEM.DATE_CREATED%type,
		LAST_UPDATED	RUN_CONTEXT_ITEM.LAST_UPDATED%type,
		MODIFIED_BY	RUN_CONTEXT_ITEM.MODIFIED_BY%type
		);
...
TYPE R_unit_conversion IS RECORD (
		FROM_UNIT	UNIT_CONVERSION.FROM_UNIT%type,
		TO_UNIT	UNIT_CONVERSION.TO_UNIT%type,
...
-- result, run_context_item, result_hierarchy are drop and replace tables where we only
...
--                       aco_cursor);
    ...
    as
        ...
        where ASSAY_STATUS = ar_src.ASSAY_STATUS
          and ASSAY_TITLE = ar_src.ASSAY_TITLE
...
--            return false;
    ...
    exception
        ...
        then
            log_error(-20001, 'target assay_context_id is null', 'map_measure',
...
--            return false;
    ...
    function map_run_context_item
        (ar_src in r_run_context_item,
         aro_trgt out r_run_context_item)
         return boolean
    as
        ln_trgt_experiment_id    number := null;    -- preset these in case they are skipped below
        ...
        ln_trgt_grp_run_context_id    number := null;    -- preset these in case they are skipped below
        ln_trgt_run_context_item_id    number := null;
        --ln_trgt_external_reference_id    number;
        le_experiment_id_null   exception;
    ...
    begin
        -- there are two possible FK in run_context_item, 1 to Experiment and 1 to Result
        get_mapping_id(pv_src_schema, 'EXPERIMENT', ar_src.experiment_id, ln_trgt_experiment_id);
        ...
        if ar_src.group_run_context_id != ar_src.run_context_item_id
        then
            get_mapping_id(pv_src_schema, 'RUN_CONTEXT_ITEM', ar_src.group_run_context_id, ln_trgt_grp_run_context_id);
            -- this is happening because the parent run_context has not yet been saved (and thus is not in identifier_mapping)
        ...
        get_mapping_id(pv_src_schema, 'RUN_CONTEXT_ITEM', ar_src.run_context_item_id, ln_trgt_run_context_item_id);

        aro_trgt := ar_src;
        aro_trgt.run_context_item_ID := ln_trgt_run_context_item_id;
        aro_trgt.GROUP_RUN_CONTEXT_ID := ln_trgt_grp_run_context_id;
        aro_trgt.EXPERIMENT_ID := ln_trgt_experiment_id;
        aro_trgt.RESULT_ID := ln_trgt_result_id;
        aro_trgt.ATTRIBUTE_ID := ln_trgt_attribute_id;
        aro_trgt.VALUE_ID := ln_trgt_value_id;
    ...
    exception
        when le_experiment_id_null
        then
            log_error(-20001, 'target Experiment_id is null', 'map_run_context_item',
                 'source experiment_id = ' || to_char(ar_src.experiment_id) || 'source = ' || pv_src_schema);
            return false;
        when le_result_id_null
        then
            log_error(-20001, 'target Result_id is null', 'map_run_context_item',
                 'source result_id = ' || to_char(ar_src.result_id));
            return false;
        when le_element_id_null
        then
            log_error(-20001, 'target Attibute or Value_id is null', 'run_context_item',
                 'source attribute_id = ' || to_char(ar_src.attribute_id)
                 || ', value_id = ' || to_char(ar_src.value_id));
            return false;
        when others
        then
            log_error(sqlcode, sqlerrm, 'map_run_context_item');
            return false;
    end map_run_context_item;
    ...
    -- we have versions for each of the 'entity' tables
        ...
        select experiment_id from external_reference
         ...
         where experiment_id = cr_experiment_id
         union all
         select experiment_id from run_context_item
         where experiment_id = cr_experiment_id;
    ...
    begin
        ...
        then
            ...
            loop
                ...
                cleanout_data_mig ('RUN_CONTEXT_ITEM_experiment', rec_exp.experiment_id, av_src_schema);
        ...
        then
            ...
            loop
                cleanout_data_mig ('RUN_CONTEXT_ITEM_result', rec_result.result_id, av_src_schema);
        ...
        elsif av_table_name = 'RUN_CONTEXT_ITEM_result'
        then
        ...
        elsif av_table_name = 'PROJECT_CONTEXT_ITEM_project'
        then
            delete from project_context_item
            where project_id = an_id;
    ...
    begin
        ...
        -- if it exists, update the assay record
            update assay set
                ...
                ASSAY_TITLE = ar_row.ASSAY_TITLE,
...
--                ar_row.STAGE_ID,
    ...
    as
        ...
        where assay_id = ar_row.assay_id
          ...
          and nvl(assay_context_id, -100) = nvl(ar_row.assay_context_id, -100)
    ...
    procedure save_run_context_item
        (ar_row in r_run_context_item,
         an_src_run_context_item_id   in  number,
         ano_run_context_item_id    out number)
    as
        ln_trgt_run_context_item_id   number;
        lv_statement    varchar2(4000);

    begin
        ln_trgt_run_context_item_id := ar_row.run_context_item_id;
        lv_statement := to_char(ar_row.GROUP_RUN_CONTEXT_ID)
            || ', ' || to_char(ar_row.EXPERIMENT_ID)
            || ', ' || to_char(ar_row.RESULT_ID)
            || ', ' || ar_row.DISCRIMINATOR
            || ', ' || to_char(ar_row.ATTRIBUTE_ID)
            || ', ' || to_char(ar_row.VALUE_ID)
            || ', ' || ar_row.EXT_VALUE_ID
            || ', ' || ar_row.QUALIFIER
         ...
         if ln_trgt_run_context_item_id is not null
        then

        -- if it exists, update the run_context_item record
            update run_context_item set
                GROUP_RUN_CONTEXT_ID = nvl(ar_row.GROUP_RUN_CONTEXT_ID,ln_trgt_run_context_item_id),
                EXPERIMENT_ID = ar_row.EXPERIMENT_ID,
                RESULT_ID = ar_row.RESULT_ID,
                DISCRIMINATOR = ar_row.DISCRIMINATOR,
                ATTRIBUTE_ID = ar_row.ATTRIBUTE_ID,
                VALUE_ID = ar_row.VALUE_ID,
                EXT_VALUE_ID = ar_row.EXT_VALUE_ID,
                QUALIFIER = ar_row.QUALIFIER,
                ...
                LAST_UPDATED = ar_row.last_updated,
                MODIFIED_BY = ar_row.MODIFIED_BY
            where run_context_item_id = ln_trgt_run_context_item_id;

           if sql%rowcount < 1
           then
              ln_trgt_run_context_item_id := null;
           else
              save_mapping(pv_src_schema, 'RUN_CONTEXT_ITEM',
                    an_src_run_context_item_id,
                    ln_trgt_run_context_item_id);
              --log_statement('run_context_item', ln_trgt_run_context_item_id, 'UPDATE', lv_statement);
           end if;

        end if;

        if ln_trgt_run_context_item_id is  null
        then
        -- if not insert a new row
            select run_context_item_id_seq.nextval
            into ln_trgt_run_context_item_id
            from dual;
            ...
            insert into run_context_item (
                ...
                ln_trgt_run_context_item_id,
                nvl(ar_row.GROUP_RUN_CONTEXT_ID, ln_trgt_run_context_item_id),
                ar_row.EXPERIMENT_ID,
                ar_row.RESULT_ID,
                ar_row.DISCRIMINATOR,
                ar_row.ATTRIBUTE_ID,
            ...
            --log_statement('run_context_item',ln_trgt_run_context_item_id,
            --      'INSERT', lv_statement);
        end if;

        ano_run_context_item_id := ln_trgt_run_context_item_id;

    exception
        when others
        then
            log_error (sqlcode, sqlerrm, 'save_run_context_item','source run_context_item_id='
                    || to_char(an_src_run_context_item_id)
                    || ' target id=' || to_char(ln_trgt_run_context_item_id) );
    end save_run_context_item;

    procedure get_run_context_item
        (an_result_id   in  number,
         an_experiment_id    in  number)
    as
        cur_run_context_item  r_cursor;
        ln_trgt_run_context_item_id   number;    -- for the return value
        lr_trgt_run_context_item  r_run_context_item;
        lr_src_run_context_item  r_run_context_item;
        le_FKs_null     exception;

    begin
--        dbms_output.put_line ('get_run_context_item, result_id=' ||to_char(an_result_id)
--                || ', experiment=' || to_char(an_experiment_id));
        if an_result_id is not null
        then
            open_src_cursor(pv_src_schema, 'run_context_item', an_result_id, cur_run_context_item);
        elsif an_experiment_id is not null
        then
            open_src_cursor(pv_src_schema, 'run_context_item_experiment', an_experiment_id, cur_run_context_item);
        else
            -- both the FK are null - this shouldn't be possible
            raise le_fks_null;
        end if;

        loop
            fetch cur_run_context_item into lr_src_run_context_item;
            exit when cur_run_context_item%notfound;
--            dbms_output.put_line ('get_run_context_item, result_id=' ||to_char(an_result_id)
--                || ', i/p experiment=' || to_char(an_experiment_id)
--                || ', QA experiment=' || to_char(lr_src_run_context_item.experiment_id)
--                || ', QA result=' || to_char(lr_src_run_context_item.result_id)
--                || ', modified_by=' || lr_src_run_context_item.modified_by);

            if map_run_context_item (lr_src_run_context_item, lr_trgt_run_context_item)
            then

                save_run_context_item (lr_trgt_run_context_item,
                            lr_src_run_context_item.run_context_item_id,
                            ln_trgt_run_context_item_id);
            end if;
        end loop;

        close cur_run_context_item;

    exception
        when le_FKs_null
        then
            if cur_run_context_item%isopen
            then
                close cur_run_context_item;
            end if;
            log_error(-20001, 'both FK (exprt and rslt) are null', 'get_run_context_item',
                'src ID='|| to_char(lr_src_run_context_item.run_context_item_id));
        when others
        then
            if cur_run_context_item%isopen
            then
            ...
            log_error(sqlcode, sqlerrm, 'get_run_context_item');
    end get_run_context_item;
    ...
    begin
        ...
        loop
            ...
            then
                ...
                get_run_context_item(lr_src_result.result_id, lr_src_result.experiment_id);
    ...
    begin
        ...
        loop
            ...
            then
                ...
                cleanout_data_mig ('RUN_CONTEXT_ITEM_experiment', ln_trgt_experiment_id, pv_src_schema);

                -- catch the items that are not coupled to results
                get_run_context_item( null, lr_src_experiment.experiment_id );
