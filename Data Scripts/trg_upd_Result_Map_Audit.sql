PROMPT CREATE OR REPLACE TRIGGER upd_result_map_audit_date
CREATE OR REPLACE TRIGGER upd_result_map_audit_date
 BEFORE update OR insert
     ON result_map
     FOR EACH ROW

WHEN (new.modified_by is null)
BEGIN
...
PROMPT cREATE INDEXES FOR THE TRIGGER
...
CREATE OR REPLACE TRIGGER fk_result_map_element
 BEFORE update
        --OR insert
     OF resulttype, contextitem, stats_modifier, attribute1, attribute2, concentrationUnit
     ON result_map
     FOR EACH ROW
...
ALTER TRIGGER FK_RESULT_MAP_ELEMENT DISABLE;
