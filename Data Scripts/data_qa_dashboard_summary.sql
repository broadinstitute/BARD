...
select c.project_uid,
       ...
       case when (prj.project_status = 'Approved') then 'QA Complete'
            when (prj.project_id is not null and qs.name is not null) then initcap(qs.name)
            when (prj.project_id is not null) then 'Loaded'
            else 'Not Yet Loaded' end as status
...
and   ps.QA_STATUS_ID = qs.ID(+)
and   ps.ID(+) = prj.PROJECT_ID
;
...
select distinct c.aid,
	   ...
	   ld.load_date,
	   qad.qa_complete_date,
       case when (e.EXPERIMENT_STATUS = 'Approved') then 'QA Complete'
            when (e.EXPERIMENT_ID is not null) then 'Loaded'
            else 'Not Yet Loaded' end as status	   
from cars c,
...
-- assay view
-- includes estimated remaining assays to load (via a 50% experiment to assay compression ratio)
create or replace view vw_dashbd_prod_assay as
select distinct a.assay_id,
       decode(c.assay_center,'SRI Screening','SRI',
	                         'Vanderbilt Chemistry','Vanderbilt',
	                         'Hopkins Screening','Hopkins',c.assay_center) as assay_center,
	   a.assay_name,
	   ld.load_date,
	   qad.qa_complete_date,
       case when (a.ASSAY_STATUS = 'Approved') then 'QA Complete'
            when (a.assay_ID is not null) then 'Loaded'
            else 'Not Yet Loaded' end as status	   
from cars c,
     (select distinct experiment_id, ltrim(ext_assay_ref, 'aid=') as aid
	  from bard_cap_prod.external_reference er
	  ...
	  and experiment_id is not null) ea,
     bard_cap_prod.experiment e,
     bard_cap_prod.assay a,
     (select arl.primary_key as assay_id, min(audit_timestamp) as Load_Date
	  from bard_cap_prod.audit_column_log acl,
	       bard_cap_prod.audit_row_log arl
	  where acl.audit_id = arl.audit_id
	  and arl.table_name = 'ASSAY'
	  and acl.column_name = 'READY_FOR_EXTRACTION'
	  and arl.table_owner = 'BARD_CAP_PROD'
	  and arl.action = 'UPDATE'
	  group by arl.primary_key) ld,
	 (select a.assay_id, max(audit_timestamp) as QA_Complete_Date
	  from bard_cap_prod.audit_column_log acl,
	       bard_cap_prod.audit_row_log arl,
	       bard_cap_prod.assay a
	  where acl.audit_id = arl.audit_id
	  ...
	  and arl.table_owner = 'BARD_CAP_PROD'
	  and arl.action = 'UPDATE'
	  and arl.primary_key = a.ASSAY_ID
	  and a.ASSAY_STATUS = 'Approved'
	  group by a.assay_id) qad	  
where c.SUSPECT is null
and e.ASSAY_ID(+) = a.assay_id
and c.aid(+) = ea.aid
and ea.experiment_id(+) = e.experiment_id
and a.ASSAY_ID = ld.assay_id(+)
and a.ASSAY_ID = qad.assay_id(+)
and a.assay_status <> 'Retired'
;
