...
class BootStrap {
    ...
    def createViews(final Sql sql) {
      ...
      INSERT INTO UNIT_ELEMENT (ELEMENT_ID, ELEMENT_STATUS, LABEL, DESCRIPTION, ABBREVIATION, SYNONYMS, UNIT, BARD_URI, EXTERNAL_URL, READY_FOR_EXTRACTION, VERSION, DATE_CREATED, LAST_UPDATED, MODIFIED_BY)
        ...
        sql.execute(STAGE_ELEMENT)
        sql.execute(ASSAY_DESCRIPTORE_ELEMENT)
        sql.execute(BIOLOGY_DESCRIPTOR_ELEMENT)
        sql.execute(INSTANCE_DESCRIPTOR_ELEMENT)
        sql.execute(UNIT_ELEMENT)
...
UNIT VARCHAR2(128)
    ...
    def insertExperiments(final Sql sql) {
        ...
        sql.execute "INSERT INTO RUN_CONTEXT_ITEM (DISCRIMINATOR,RUN_CONTEXT_ITEM_ID,RESULT_ID,ATTRIBUTE_ID,VALUE_DISPLAY,VALUE_NUM,QUALIFIER,VERSION,DATE_CREATED) VALUES ('Result',5,533,370,'66',66,'%',0,SYSDATE)"
