...
class ProjectQueries {
	...
	final static String PROJECT_CONTEXTS = "SELECT PC.CONTEXT_NAME ContextName FROM PROJECT_CONTEXT PC WHERE PC.PROJECT_ID = ? AND PC.CONTEXT_GROUP = ?"
	final static String PROJECT_CONTEXT_ITEMS = "SELECT E.LABEL AttributeLabel, PCI.VALUE_DISPLAY ValueDisplay FROM PROJECT_CONTEXT_ITEM PCI, ELEMENT E WHERE PCI.ATTRIBUTE_ID = E.ELEMENT_ID AND PCI.PROJECT_CONTEXT_ID IN(SELECT PC.PROJECT_CONTEXT_ID ContextID FROM PROJECT_CONTEXT PC WHERE PC.PROJECT_ID = ? AND PC.CONTEXT_GROUP = ? AND PC.CONTEXT_NAME = ?)";
