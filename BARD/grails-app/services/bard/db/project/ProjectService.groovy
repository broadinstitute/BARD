...
//     */
//    @PreAuthorize("hasPermission(#id, 'bard.db.project.Project', admin) or hasRole('ROLE_BARD_ADMINISTRATOR')")
//    void linkProjectExperiment(Experiment fromExperiment, Experiment toExperiment, Long id) {
...
//
    /**
     * Add link between two project-experiments that are associated with the project. The candidate link can not be duplicate with any existing one.
     * The candidate link can not result any cycle of graph.
    ...
    void linkProjectExperiment(ProjectExperiment fromProjectExperiment, ProjectExperiment toProjectExperiment, Long id) {
        ...
        if (ps) // do not want to add one already exists
