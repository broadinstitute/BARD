    ...
    begin
        ...
        loop
            ...
            select ASSAY_CONTEXT_ID,
                ...
                LAST_UPDATED,
                MODIFIED_BY
            from data_mig.assay_context_measure dmacm
            where (measure_id, assay_context_id ) NOT IN
                  (SELECT measure_id, assay_context_id
                    FROM assay_context_measure acm)
              AND EXISTS (SELECT 1
                  FROM measure m
                  WHERE m.measure_id = dmacm.measure_id)
              AND EXISTS (SELECT 1
                  FROM assay_context ac
                  WHERE ac.assay_context_id = dmacm.assay_context_id);
